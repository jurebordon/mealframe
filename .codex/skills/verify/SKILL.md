...
name: verify
description: >-
  Validate SpecFlow documentation consistency and project health. Checks required
  files exist, validates ROADMAP structure and feature tags, validates SESSION_LOG
  format, checks configuration, and reports issues. Read-only, no modifications.
...
which  2>/dev/null || echo "WARN: Test command binary not found"
which  2>/dev/null || echo "WARN: Build command binary not found"
which  2>/dev/null || echo "WARN: Lint command binary not found"
```
...
- [ ] Current branch follows naming convention (`feat/description`)
- [ ] Not behind remote:
  ```bash
